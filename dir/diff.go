...
DIR_COMPARE_LOOP:
	for {
		...
		case originalFilesIndex >= originalFilesLen:
			// We are at the end of the list for originalFiles any files in originalFiles are
			// not in newFiles. We treat this as a delete from originalFiles.

			// do something
		case newFilesIndex >= newFilesLen:
			// We are at the end of the list for newFiles. Any files in originalFiles were added
			// and are not in newFiles. We treat this as a add.
